# Developer Guide — Stanbic FX & Cross-Border Transfer Portal

Welcome to the FX Modernization programme. This guide covers local onboarding,
...
```bash
git clone https://github.com/enterprise-org/stanbic-fx-portal.git
cd stanbic-fx-portal
...
Every change traces to a backlog **Item ID** (e.g. `FX-101`).

| Artifact | Convention | Example |
|----------|-----------|---------|
| Feature branch | `feature/FX-<id>-<kebab-slug>` | `feature/FX-101-fee-breakdown` |
| Defect branch | `fix/FX-<id>-<kebab-slug>` | `fix/FX-214-gbp-rounding` |
| Commit | `feat(fx): #<issue> <imperative>` | `feat(fx): #101 add live fee breakdown` |
...
- **Selectors**: Playwright targets `data-testid` exclusively — kebab-case with
  the `fx-` prefix (`fx-amount-input`, `fx-submit-button`). Never CSS classes or text.
- **Page Object Model**: page interactions live in `e2e/pages/`; specs in
  `e2e/` stay declarative. Extend `FxTransferPage` rather than inlining locators.
...
1. Pick an item from the **Stanbic FX Modernization** project board.
2. `git switch -c feature/FX-<id>-<slug>` from a fresh `main`.
